...
2. **Inner Loop**: Compares adjacent elements and swaps them if they're in the wrong order
...
- **Registers**: Using `%eax`, `%ebx`, `%ecx`, `%edx`, `%esi`, `%edi` for operations
- **Memory Access**: Loading and storing data from memory addresses
- **Control Flow**: Conditional jumps (`jle`, `jge`, `jmp`) for loops and conditionals
