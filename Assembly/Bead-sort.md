...
inner_loop:
    ...
    movl    (%eax), %ebx       # ebx = array[j]
    movl    4(%eax), %edi      # edi = array[j+1]
...
- **Conditional branching** using `cmp` and `jle`/`jg` instructions
- **Memory addressing** and pointer arithmetic
- **Register usage** for temporary storage
- **Loop control** with decrement and jump instructions
