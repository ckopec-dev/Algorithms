...
outer_loop:
    ...
    subl %esi, %edx                # Calculate (n-1-i)
    
inner_loop:
    cmpl %edi, %edx                # Compare j with (n-1-i)
    jge next_outer                 # If j >= (n-1-i), go to next outer iteration
...
5. **Loop Control**: Uses conditional jumps (`jge`, `jle`) for loop management
