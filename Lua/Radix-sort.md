...
function radix_sort(arr)
    ...
    local exp = 1
    while math.floor(max_num / exp) > 0 do
...
function counting_sort_by_digit(arr, exp)
    local n = #arr
    local output = {}
    local count = {}
...
2. **Digit-by-Digit Sorting**: We process each digit from least significant to most significant
3. **Counting Sort**: For each digit position, we use counting sort to maintain stability
...
The algorithm is stable and works efficiently for sorting integers or fixed-length strings.
