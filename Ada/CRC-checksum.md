# CRC Checksum Algorithm in Ada

Here's an example implementation of a CRC-32 checksum algorithm in Ada:

```ada
with Ada.Text_IO; use Ada.Text_IO;
with Ada.Integer_Text_IO; use Ada.Integer_Text_IO;

procedure CRC_Example is
   
   -- CRC lookup table for polynomial 0xEDB88320 (IEEE 802.3 standard)
   ...
   function CRC_32(Data : String) return Integer is
      CRC : Integer := 16#FFFFFFFF#;
      Index : Natural;
      Byte : Integer;
   begin
      for I in Data'Range loop
         Byte := Character'Pos(Data(I));
         Index := (CRC xor Byte) and 16#FF#;
         CRC := (CRC / 16#100#) xor CRC_Table(Index);
      end loop;
...
begin
   Test_CRC := CRC_32(Test_String);
   
   Put_Line("String: """ & Test_String & """");
   Put_Line("CRC-32: 0x" & Integer'Image(Test_CRC));
   Put_Line("CRC-32 (decimal): " & Integer'Image(Test_CRC));
   
   -- Verify with known value
   Put_Line("Expected CRC-32 for ""Hello, World!"": 0x1C9F4820");
...
1. **Standard CRC-32 Polynomial**: Uses the IEEE 802.3 polynomial (0xEDB88320)
2. **Lookup Table**: Pre-computed table for efficient calculation
3. **Bit Manipulation**: Proper handling of bitwise operations
4. **String Input**: Accepts Ada strings as input data
5. **Standard Output**: Displays both hexadecimal and decimal representations

## How it Works:

1. Initialize CRC to all 1s (0xFFFFFFFF)
2. For each byte in the input string:
   - XOR the current byte with the lowest 8 bits of CRC
   - Use the result as an index into the lookup table
   - XOR the table value with the upper 24 bits of CRC
3. Final result is XORed with 0xFFFFFFFF

This implementation follows the standard CRC-32 algorithm used in many networking protocols and file formats.
